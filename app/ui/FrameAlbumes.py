...
class FrameAlbumes(tk.Frame):
    ...
    def __init__(self, master, reproductor, biblioteca, *args, **kwargs):
        ...
        self.lista_albumes = biblioteca.obtenerListaAlbumes()
        self.lista_frames_album = Lista()
        
        
        
        # Crear el label en la parte superior con el nombre del Frame
        label_titulo = tk.Label(self, text="Albumes", font=("Arial", 35, "bold"), bg="black", fg="white")
    ...
    def agregar_albums(self):
        lista_albums = self.lista_albumes
        
        longitud = lista_albums.obtenerLongitud()
        contador = 1
        while contador <= longitud: 
            album = lista_albums.obtenerPorIndice(contador).obtenerDato()
            if album != None:
